...
- Artifacts for specific runs will be uploaded to a directory with a
  ...
  `gs://aiap-team-7-project-jokebot-artifacts/mlflow-tracking-server/team_7/<MLFLOW_EXPERIMENT_UUID>`.
...
- `aiap_team_7_project_jokebot`
- `aiap_team_7_project_jokebot_fastapi`
...
=== "Windows PowerShell"
    ...
    $ conda activate aiap-team-7-project-jokebot
    $ cd src
    $ uvicorn aiap_team_7_project_jokebot_fastapi.main:APP
    ```
...
`src/aiap_team_7_project_jokebot_fastapi/config.py`. This class contains
...
`src/aiap_team_7_project_jokebot_fastapi/config.py`:
```python
...
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/team_7/fastapi-server:0.1.0 \
        --build-arg PRED_MODEL_UUID="$PRED_MODEL_UUID" \
        -f docker/aiap-team-7-project-jokebot-fastapi.Dockerfile \
...
=== "Linux/macOS"
    ...
    $ docker run --rm -p 8080:8080 \
        ...
        asia.gcr.io/$GCP_PROJECT_ID/team_7/fastapi-server:0.1.0
    ```
...
```bash
$ docker push asia.gcr.io/$GCP_PROJECT_ID/team_7/fastapi-server:0.1.0
```
...
=== "Local Machine"
    ...
    ```bash
    $ kubectl port-forward service/fastapi-server-team-7-svc 8080:8080 --namespace=polyaxon-v1
...
!!! attention
    ...
    === "Local Machine"
        ...
        ```bash
        $ kubectl delete fastapi-server-team-7-deployment --namespace=polyaxon-v1
        $ kubectl delete fastapi-server-team-7-svc --namespace=polyaxon-v1
        ```
